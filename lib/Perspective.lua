...
persp.glsl=love.graphics.newShader[[
...
persp.gl_send = persp.glsl.send
persp.q = love.graphics.polygon
persp.setShader = love.graphics.setShader

--module(...)

--turn shader on before drawing any quads
function persp.on()
  persp.setShader( persp.glsl )
end
...
function persp.tex( origin, size )
	--predefined as ( 0,0 )
	persp.gl_send( persp.glsl, "p0", origin )
	--predefined as ( 1,1 )
	persp.gl_send( persp.glsl, "rep", size )
end

--draw quad
--vertices in clockwise order --format ( x,y )
--top-left of texture is "v1", top-right is "v2"
function persp.quad( img, v1,v2,v3,v4 )
  persp.setShader( persp.glsl )
	--set texture
	persp.gl_send( persp.glsl, "img", img )
	--send vertices to calculate perspective
	persp.gl_send( persp.glsl, "vv", v2,v3,v4,v1,v1 )
	--generate mesh to draw into
	persp.q( "fill", v1[1],v1[2],v2[1],v2[2],v3[1],v3[2],v4[1],v4[2] );
  persp.setShader(  )
end
